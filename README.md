...
- 方向:四页统一为 **pixel console(游戏机)风格**——展示页做成游戏机样式,边缘与交互换成像素掌机 UI
...
版本线:v10.3 后端基线(tag `baseline/v10.3-stable`)→ v11 Compose 像素四页 + 微信式左滑删除 → **v12 游戏机风格重构(进行中)**

> 注:2026-09-08 转公开前,已从全部 git 历史移除 `backups/`(本机数据库快照、语音包音频)与 `.gradle/`、`app/build/` 构建产物,提交号已随之重写;早期文档中引用的旧提交号对应本地留存的完整历史备份。
