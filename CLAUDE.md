...
## 13. 全前端重构（2026-09-08 用户新指令，覆盖旧视觉结构约束）
用户明确否定小幅调整，授权整个前端、横屏与交互重构。允许替换既有页面结构和视觉组件；保留蓝金品牌、少量像素身份、零动画与核心契约。旧单列Editor/巨型时钟/禁止面板的结构约束不再适用。先完成完整可运行前端再展示，不以旧的小样批准门槛阻塞本次授权；最终视觉通过仍须先生实际验收。设备未保存草稿保护继续有效。

## 14. 像素游戏机方向（2026-09-08 最新用户指令）
用户否定12.1安静工具台视觉，明确要求横竖屏统一游戏像素风格，页面具有游戏机机身、边缘和按键设计。停止继续打磨/交付旧sans工作台方案。保留已修复交互和双栏能力，但视觉改为像素游戏机；按钮必须对应真实操作，禁止假功能。先做整机外壳和主要页面实际运行样张，最终仍由用户视觉验收。冻结核心/资产/草稿保护继续有效。
